...
func SeedLinkedInLane(ctx context.Context, db Database) error {
	...
	if err := db.UpsertProvider(ctx, Provider{
		...
		Cost: 0.10, Quality: 0.95, LatencyMs: 100, Worker: provManualInbox,
	...
	if err := db.UpsertProvider(ctx, Provider{
		...
		Cost: 0.30, Quality: 0.90, LatencyMs: 5000, Worker: provBrightDataLinked,
		Constraints: []byte(`{"accepts":["linkedin"]}`), Enabled: true,
	...
	if err := db.UpsertProvider(ctx, Provider{
		...
		Cost: 0.20, Quality: 0.85, LatencyMs: 1000, Worker: provExtrairLinked,
		Constraints: []byte(`{"accepts":["linkedin"]}`), Enabled: true,
	}); err != nil {
		return err
	}
