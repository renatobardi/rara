...
func (d *pgxDatabase) getYouTubeChannelConfig(ctx context.Context, id int, apiID string) (map[string]string, bool, error) {
	...
	if err != nil {
		return nil, false, fmt.Errorf("get source config %q: %w", apiID, err)
	}
	return map[string]string{"channel_id": chID, "channel_name": name}, true, nil
}

func (d *pgxDatabase) getYouTubePlaylistConfig(ctx context.Context, id int, apiID string) (map[string]string, bool, error) {
	var plID string
	err := d.conn.QueryRow(ctx,
	...
	return map[string]string{"playlist_url": "https://www.youtube.com/playlist?list=" + plID}, true, nil
}

func (d *pgxDatabase) getPodcastConfig(ctx context.Context, id int, apiID string) (map[string]string, bool, error) {
	...
	return map[string]string{"feed_url": feedURL, "title": title}, true, nil
}

func (d *pgxDatabase) getFeedSourceConfig(ctx context.Context, id int, kind, apiID string) (map[string]string, bool, error) {
	var name, endpoint string
	err := d.conn.QueryRow(ctx,
		`SELECT name, COALESCE(endpoint,'') FROM feed_sources WHERE id=$1 AND source_type=$2 AND deleted_at IS NULL`, id, kind,
	).Scan(&name, &endpoint)
	if errors.Is(err, pgx.ErrNoRows) {
		return nil, false, nil
	}
...
func (d *pgxDatabase) getLinkedInProfileConfig(ctx context.Context, id int, apiID string) (map[string]string, bool, error) {
	...
	return map[string]string{"profile_url": profileURL}, true, nil
}

func (d *pgxDatabase) getEmailSourceConfig(ctx context.Context, id int, apiID string) (map[string]string, bool, error) {
	var exists bool
	err := d.conn.QueryRow(ctx,
		`SELECT true FROM email_sources WHERE id=$1 AND deleted_at IS NULL`, id,
	).Scan(&exists)
	if errors.Is(err, pgx.ErrNoRows) {
		return nil, false, nil
	}
...
func (d *pgxDatabase) GetSourceConfig(ctx context.Context, apiID string) (map[string]string, bool, error) {
	...
	case "youtube_channel":
		return d.getYouTubeChannelConfig(ctx, id, apiID)
	case "youtube_playlist":
		return d.getYouTubePlaylistConfig(ctx, id, apiID)
	case "podcast":
		return d.getPodcastConfig(ctx, id, apiID)
	case "rss", "html", "hn":
		return d.getFeedSourceConfig(ctx, id, kind, apiID)
	case "linkedin_profile":
		return d.getLinkedInProfileConfig(ctx, id, apiID)
	case "email":
		return d.getEmailSourceConfig(ctx, id, apiID)
	default:
