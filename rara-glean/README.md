# rara-glean — the already-text extractor

`rara-glean` is the **to-text worker for the text lanes** of the rara 2.0 pipeline: a bridge-total
**claim-worker** on the [`rara-addon`](../rara-addon) SDK. Some sources arrive as text, not audio —
an **email body**, a pasted **LinkedIn post** — so they need no ASR. They need **normalization**:
strip the noise the source carries and leave the human-written message the gates and distill judge.
...
One codebase, **two providers** by config (`GLEAN_PROVIDER`):

- **`extrair-email`** — strips HTML, the `-- ` signature, and quoted-reply history from an email body
  (`emails.body`, keyed on `message_id`).
- **`extrair-linkedin`** — normalizes a pasted post: strips HTML if present, collapses blank runs,
  trims (`linkedin_posts.body`, keyed on `url`). A post has no signature/quote, so this is lighter.

Codebases ≪ providers — the handler picks the cleaner and the `source_type` by the item's lane.
...
export GLEAN_PROVIDER=extrair-email   # or extrair-linkedin
make test              # zero-I/O unit tests (pure cleaners + handler with a mock store)
make build             # local binary (glean-job)
...
glean owns **no** table of its own. It **reads** the collectors' `emails` / `linkedin_posts` and
**writes** the to-text owner's (rara-scribe's) shared `transcripts` table — the universal to-text
store, shared by design. All live in the one shared Neon database, so there is **no `migrations/`**
here and no `database-glean.yml`.
